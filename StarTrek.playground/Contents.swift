...
var cnt = humanOfficers.count
print(cnt)


// Q1: What percentage of humans are female?
let percentFemaleHuman = Float(femaleOfficers.count)/Float(humanOfficers.count)
percentFemaleHuman
...
let femaleSort = femaleOfficers.sorted()
femaleSort
...
let firstOffVulcan: [Officer] = firstOfficers.map{Officer(name: $0, ship: $1)}.filter{vulcanOfficers.contains($0.name)}
print(firstOffVulcan.map{$0.name})



// Q7: How many these Vulcan first officers have names shorter than 5 characters?
let cntFirstOffVulcan = firstOffVulcan.filter{$0.name.count < 5}.count
print(cntFirstOffVulcan)
